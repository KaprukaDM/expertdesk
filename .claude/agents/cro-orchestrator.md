...
3. Dispatch the selected specialists via the Agent tool — `cro-funnel-auditor` runs before `cro-ux-reviewer`'s re-scan and `cro-ab-test-strategist`'s roadmap since both reference its findings; `cro-sins-auditor`, `cro-tracking-auditor`, and `cro-competitor-benchmark` are independent.
